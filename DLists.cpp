...
int DLIndex::comp(string pVal, PDATA pB)
{
    ...
    switch (aType) {
        ...
        case ECampos::contrasena:
            ...
            break;
        case ECampos::tarjeta:
            if (pVal < pB->sTarjeta) lRes = -1;
            else if (pVal == pB->sTarjeta) lRes = 0;
            else lRes = 1;
            break;
        case ECampos::saldo:
            lValD = stod(pVal);
...
void DList::push_back(string pNombre, string pApellido, 
     string pUsuario, string pContrasena, string pTarjeta, double pSaldo)
{
    if (aHead == NULL) {
...
void DList::del(string pNombre, string pApellido, string pUsuario, string pContrasena, string pTarjeta, double pSaldo)
{
    if (aHead) {
...
void DList::read(string pPath)
{
    ...
    if (lTbl.size() > 0) {
        for (size_t lCol = 0; lCol < lTbl.size(); lCol++) {
            push_back(lTbl[lCol][0], lTbl[lCol][1], lTbl[lCol][2], lTbl[lCol][3], lTbl[lCol][4], stod(lTbl[lCol][5]));
        }
    }
} // read
...
PDNODE DList::find(string pNombre, string pApellido, string pUsuario, string pContrasena, string pTarjeta, double pSaldo)
{
